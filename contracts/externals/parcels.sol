...
contract OldParcel is ERC721Token, Ownable {
  struct BoundingBox {
    int16 x1;
    int16 y1;
    int16 z1;
  ...
  function setContentURI(uint256 _tokenId, string memory _uri) public {
    require(exists(_tokenId));
    require(ownerOf(_tokenId) == msg.sender);
    contentURIs[_tokenId] = _uri;
  }

  function contentURI(uint256 _tokenId) public view returns (string memory) {
    require(exists(_tokenId));
